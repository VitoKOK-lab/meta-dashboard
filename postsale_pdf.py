...
def ensure_fonts():
    ...
    reg = os.path.join(FONT_DIR, 'NotoSansTC-Bold.ttf')
    bold = os.path.join(FONT_DIR, 'NotoSansTC-Black.ttf')
...
def kpi_block(kpi, styles, story):
    ...
    story.append(Paragraph(
        '{}｜活動 <b>{}</b>｜對應影片 <b>{}</b>｜總播放 <b>{}</b>｜'
        '加購 <font color="{}"><b>{}</b></font>'.format(
            kpi['period'], fmt(kpi['nCamps']), fmt(kpi['matched']),
            fmt_wan(kpi['plays']), GOOD, fmt(kpi['slAddons'])),
        styles['kpi']))
...
def build_product_pdf(glist, link_map, kpi, out_path):
    ...
    for rank, g in enumerate(glist, 1):
        ...
        summary = Paragraph(
            '總播放 {}｜總觸及 {}｜SL留言 {}｜加購 <font color="{}">{}</font>'.format(
                fmt(g['plays']), fmt(g['reach']), fmt(g['slComments']),
                GOOD, fmt(g['slAddons'])),
            styles['gsum'])
        ...
        for idx, e in enumerate(g['camps']):
            ...
            sl_bits = []
            if c['slComments']:
                sl_bits.append('留言 ' + fmt(c['slComments']))
            if c['slAddons']:
                sl_bits.append('<font color="{}">加購 {}</font>'.format(GOOD, fmt(c['slAddons'])))
            if c['slSales']:
                sl_bits.append('NT$ ' + fmt(c['slSales']))
            sl = '<br/>'.join(sl_bits) or '—'
            if e['videos']:
                for k, (vid_id, v) in enumerate(e['videos']):
                    ...
                    rows.append([
                        ...
                        Paragraph('{:.1f}%'.format(er_of(v)), styles['cell']),
                ...
                rows.append([
                    ...
                    Paragraph('{}　<font color="{}" size="9.5">{}</font>'.format(
                        post_links, MUTED,
                        '照片貼文' if c['postType'] == '照片' else '無影片數據'),
                        styles['cell']),
...
def build_video_pdf(videos, link_map, kpi, out_path):
    ...
    for rank, e in enumerate(entries, 1):
        ...
        vurl = video_url(vid_id, v.get('platform'), link_map)
        vcell = '{}<br/><font color="{}" size="9.5">{}　{}</font>'.format(
            link_para(vtitle, vurl, styles['cell']),
            MUTED, plat_tag(v.get('platform') or ''), v.get('created_date') or '')
        prods = []
        for c in e['camps']:
        ...
        rows.append([
            ...
            Paragraph('<b>{}</b>'.format(fmt(v.get('plays') or 0)), styles['cell']),
            Paragraph(fmt(v.get('reach') or 0), styles['cell']),
            Paragraph('<b>{:.1f}%</b>'.format(er_of(v)), styles['cell']),
